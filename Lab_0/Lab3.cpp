...
void Lab3::run() {
	...
	imshow("gauss3", blur);
	imshow("‘ильтр Ђћозаикаї", mos3(original));
	imshow("‘ильтр апертурный коррекции", ac3(3, original));
	imshow("ћедианный фильтр", med3(original));
...
Mat Lab3::mos3(const cv::Mat& imgin) {
	...
	for (int i = 1; i < imgin.cols - 2; i += 3)
		for (int j = 1; j < imgin.rows - 2; j += 3) {
			// поиск среднего арифметического значени€ €ркости элементов под маской
			double avg = 0;
...
Mat Lab3::med3(const cv::Mat& imgin) {
	...
	for (int i = 1; i < imgin.cols - 1; i++)
		for (int j = 1; j < imgin.rows - 1; j++) {
			...
			// поиск элемента с минимальной разницей между значением €ркости и среднеарифметическим значением €ркости матрицы 3на3
			for (int ii = -1; ii <= 1; ii++) {
				for (int jj = -1; jj <= 1; jj++) {
